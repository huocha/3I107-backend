...
public class Parser {
    ...
    public void parse() throws Exception {
        ...
        IndexColumn ic1=new IndexColumn(0); // IndexColumn by the first column->numero 0 , ie: vendor_name 
        Index i1=new Index();
        i1.insert(ic1); 
        ...
        File ff=new File(workingDirectory+dataFileSearch);
        boolean fileExiste=ff.exists(); //check if the file research existe?
        PrintWriter  pw=null;
        if (!fileExiste) {
        ...
        try {
        	while (scanner.hasNext()) {
        		...
        		if (!fileExiste) pw.println(thisLine); //write new file
               
        		parseLine(thisLine,i1.getIndexCol().get(0));
        ...
        if (!fileExiste) pw.close();
        
        table.insert(i1); // add index which contain indexColumn of vendor_name to table for ex
